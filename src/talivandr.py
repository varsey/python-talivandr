...
try:
    ...
    r = requests.get('http://talivandr.site/db/talivandr_db1.json') #http://talivandr.site/db/talivandr_db1.json')
    ...
    if path.exists(datafile):
        with open(datafile, encoding="utf8") as f:
            data = json.load(f)
            print("Local DB loaded: " + str(len(data)) + " entries")
